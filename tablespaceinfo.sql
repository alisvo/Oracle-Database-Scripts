...
column MAX_PCT format 999.99 heading "MAX|PCT"
column tbspce format A30
col container for a30

select 
    substr(f.tablespace_name, 1, 30) tbspce,
    ...
    round((nvl(s.segbytes, 0) / f.maxbytes) * 100, 2) MAX_PCT,
    lower(vc.name) as container
from
    (select con_id, tablespace_name, sum(bytes) tsbytes, sum(maxbytes) maxbytes 
     from cdb_data_files 
     group by con_id, tablespace_name) f,
    (select con_id, tablespace_name, sum(bytes) segbytes 
     from cdb_segments 
